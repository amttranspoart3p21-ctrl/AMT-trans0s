...
export default function OCRPage() {
  ...
  const uploadImage = async (file: File) => {
    ...
    try {
      ...
      // Reset OCR results & stats when replacing image
      setShipments([]);
      setCoordinates({});
      setSaveResult(null);
      setScale(1);
                ...
                {entryMode === "ocr" && activeFilename && (
                  <div className="flex items-center gap-1.5">
                    ...
                    <button
                      ...
                      className="px-2 py-0.5 bg-slate-800 hover:bg-slate-700 border border-slate-700/60 text-slate-300 hover:text-white rounded-lg transition-all text-[10px] font-semibold flex items-center gap-1 cursor-pointer disabled:opacity-50"
            ...
            <div className="px-4 py-2 bg-slate-900/40 border-b border-slate-800/50 flex justify-between items-center text-xs font-semibold text-slate-400 select-none">
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => fileInputRef.current?.click()}
                  disabled={uploading || loading}
                  className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 border border-slate-700/60 text-slate-300 hover:text-white rounded-lg transition-all text-[11px] font-medium flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
                  title="Replace current register image"
                >
                  <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
