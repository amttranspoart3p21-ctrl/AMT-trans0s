...
async function testPricing() {
  try {
    ...
    // Verify other companies (ABC Company) CANNOT use Machine Parts
    let errorCaught = false;
    try {
      await calculateShipmentPricing({
        paymentCompany: abcCompany.companyName,
        fromCompany: abcCompany.companyName,
        toCompany: xyzCompany.companyName,
        fromAmtBranch: chennaiBranch.branchName,
      ...
      errorCaught = true;
      console.log("Expected Error caught for ABC Company using Machine Parts:", e.message);
    }
    console.assert(errorCaught, "Other companies must not be able to use Machine Parts");
    console.log("✅ TEST 3 PASSED: Company-Specific Package successfully isolated to XYZ Company!");
